    ...
    <div className={`fixed w-full top-0 left-0 z-50 border-b border-n-6 lg:bg-n-8/90 lg:backdrop-blur-sm ${openNavigation ? 'bg-n-8' : 'bg-n-8/90 backdrop-blur-sm'}`}>
      <div className="flex items-center px-5 lg:px-7.5  xl:px-10 max-lg:py-4">
        <a href="#hero" className="block w-[12rem] xl:mr-8 ">
          <img src={mealora} width={190} height={40} alt="mealora" />
        </a>
            ...
            {navigation.map((item) => (
              <a
                ...
                className={`block relative font-code text-2xl uppercase text-n-1 transition-colors hover:text-color-3
                  ${item.onlyMobile ? 'lg:hidden sm:hidden' : ''}
                  px-6 py-6 md:py-8 lg:-mr-0.25 lg:text-xs lg:font-semibold
                  ${item.url === hash ? 'z-2 lg:text-color-2' : 'lg:text-n-1/50'}
        ...
        <Button className="ml-auto  lg:hidden items-center  "
